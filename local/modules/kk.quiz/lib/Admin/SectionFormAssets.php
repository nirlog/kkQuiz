...
final class SectionFormAssets
{
    ...
    private static function renderScript(array $section): string
    {
        return '(() => {'
            ...
            . '};'
            . 'const addFieldGroup = (title, fieldNames) => {'
            . 'const first = fieldNames.map((name) => document.querySelector(`[name="${name}"]`)).find(Boolean);'
            . 'const row = first && (first.closest("tr") || first.parentElement);'
            . 'if (!row || document.querySelector(`[data-kk-quiz-group="${title}"]`)) return;'
            ...
            . 'setTimeout(groupAppearanceFields, 100);'
            . 'setTimeout(groupAppearanceFields, 500);'
            . '})();';
    }
