...
final class SectionFormAssets
{
    ...
    private static function renderScript(array $section): string
    {
        return '(() => {'
            ...
            . 'const field = document.querySelector(`[name="${fieldName}"]`) || document.querySelector(`[name="${fieldName}[]"]`);'
            . 'return field ? field.closest("tr") : null;'
            . '};'
            . 'const removeExistingGroups = () => {'
            . 'document.querySelectorAll("[data-kk-quiz-group]").forEach((row) => row.remove());'
            . '};'
            . 'const moveGroup = (title, fieldNames, anchorRow = null) => {'
            . 'const rows = fieldNames.map((fieldName) => findFieldRow(fieldName)).filter(Boolean);'
            . 'if (rows.length === 0) return anchorRow;'
            . 'const heading = document.createElement("tr");'
            . 'heading.dataset.kkQuizGroup = title;'
            . 'heading.innerHTML = `<td colspan="2" class="heading"><b>${title}</b></td>`;'
            . 'const table = rows[0].closest("tbody") || rows[0].parentElement;'
            . 'if (!table) return anchorRow;'
            . 'if (anchorRow && anchorRow.parentElement === table) anchorRow.insertAdjacentElement("afterend", heading);'
            . 'else rows[0].insertAdjacentElement("beforebegin", heading);'
            . 'let cursor = heading;'
            . 'rows.forEach((row) => {'
            ...
            . 'return cursor;'
            . '};'
            . 'const groupSectionFields = () => {'
            . 'removeExistingGroups();'
            . 'const groups = ['
            . '{title: "Основное", fields: ["UF_KK_TITLE", "UF_KK_SUBTITLE", "UF_KK_BUTTON_TEXT", "UF_KK_START_TEXT", "UF_KK_START_QUESTION", "UF_KK_PROGRESS_TOTAL", "UF_KK_SUCCESS_TEXT"]},'
            . '{title: "Форма заявки", fields: ["UF_KK_FORM_TITLE", "UF_KK_FORM_SUBTITLE", "UF_KK_FORM_BUTTON_TEXT", "UF_KK_FORM_FIELDS", "UF_KK_REQUIRED_FIELDS", "UF_KK_EMAIL_TO"]},'
            . '{title: "Оформление", fields: ["UF_KK_THEME", "UF_KK_MAX_WIDTH", "UF_KK_ACCENT_COLOR", "UF_KK_ACCENT_HOVER", "UF_KK_ACTIVE_COLOR", "UF_KK_PROGRESS_COLOR", "UF_KK_BORDER_RADIUS", "UF_KK_CONTAINER_RADIUS", "UF_KK_CARD_RADIUS", "UF_KK_BUTTON_RADIUS", "UF_KK_INPUT_RADIUS", "UF_KK_IMAGE_RADIUS"]},'
            . '{title: "Изображения ответов", fields: ["UF_KK_IMAGE_RATIO", "UF_KK_IMAGE_FIT"]},'
            . '{title: "Яндекс Метрика", fields: ["UF_KK_USE_METRIKA", "UF_KK_METRIKA_COUNTER_ID", "UF_KK_METRIKA_FIRST_ANSWER_GOAL", "UF_KK_METRIKA_RESULT_GOAL", "UF_KK_METRIKA_RESULT_CTA_GOAL", "UF_KK_METRIKA_PRODUCT_CLICK_GOAL", "UF_KK_METRIKA_GOAL"]},'
            . '{title: "Google Analytics", fields: ["UF_KK_USE_GA", "UF_KK_GA_MEASUREMENT_ID", "UF_KK_GA_FIRST_ANSWER_EVENT", "UF_KK_GA_RESULT_EVENT", "UF_KK_GA_RESULT_CTA_EVENT", "UF_KK_GA_PRODUCT_CLICK_EVENT", "UF_KK_GA_FORM_SUBMIT_EVENT"]},'
            . '{title: "Рекомендации", fields: ["UF_KK_USE_CATALOG", "UF_KK_CATALOG_IBLOCK_IDS", "UF_KK_CATALOG_IBLOCK_ID"]},'
            . '{title: "Popup и политика", fields: ["UF_KK_ALLOW_POPUP_URL", "UF_KK_PRIVACY_TEXT", "UF_KK_PRIVACY_URL", "UF_KK_REQUIRE_AGREEMENT"]}'
            . '];'
            . 'let cursor = null;'
            . 'groups.forEach((group) => { cursor = moveGroup(group.title, group.fields, cursor); });'
            . '};'
            . 'const init = () => { renderEmbedTab(); enhanceStartQuestionField(); groupSectionFields(); };'
            ...
            . 'setTimeout(groupSectionFields, 100);'
            . 'setTimeout(groupSectionFields, 500);'
            . '})();';
    }
