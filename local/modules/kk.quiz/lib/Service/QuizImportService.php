...
final class QuizImportService
{
    ...
    private function createSection(int $iblockId, string $code, string $name, array $quiz, array $settings): int
    {
        ...
        $fields = [
            ...
            'UF_KK_METRIKA_FIRST_ANSWER_GOAL' => (string)($settings['metrika_first_answer_goal'] ?? ''),
            'UF_KK_METRIKA_RESULT_GOAL' => (string)($settings['metrika_result_goal'] ?? ''),
            'UF_KK_METRIKA_RESULT_CTA_GOAL' => (string)($settings['metrika_result_cta_click_goal'] ?? ''),
            'UF_KK_METRIKA_PRODUCT_CLICK_GOAL' => (string)($settings['metrika_product_click_goal'] ?? ''),
            'UF_KK_USE_METRIKA' => $this->toBool($settings['use_metrika'] ?? null) ? 1 : 0,
            'UF_KK_USE_GA' => $this->toBool($settings['use_ga'] ?? null) ? 1 : 0,
            'UF_KK_GA_MEASUREMENT_ID' => (string)($settings['ga_measurement_id'] ?? ''),
            'UF_KK_GA_FIRST_ANSWER_EVENT' => (string)($settings['ga_first_answer_event_name'] ?? ''),
            'UF_KK_GA_RESULT_EVENT' => (string)($settings['ga_result_event_name'] ?? ''),
            'UF_KK_GA_RESULT_CTA_EVENT' => (string)($settings['ga_result_cta_click_event_name'] ?? ''),
            'UF_KK_GA_PRODUCT_CLICK_EVENT' => (string)($settings['ga_product_click_event_name'] ?? ''),
            'UF_KK_GA_FORM_SUBMIT_EVENT' => (string)($settings['ga_form_submit_event_name'] ?? ''),
